...
function searchBtnClicked () { 
    ...
    const site = "http://gateway.marvel.com/v1/public/characters?nameStartsWith=" + searchBar.value + "&apikey=e8ea0938ddd75570d0eb6b3ec3632fe9&ts=woahwoahwoah&hash=67207ba7d7a06807a9984b4a112d7e18";
    fetch(site)
    .then(info => info.json())
    .then(character => {
        createMessage(character)}); // calls function to create message (might end up being called later in order to as a part of an onchange function for the select instead)
}
...
function createMessage(character) { 
    ...
    if(character.data.results[0].description != "") {
        messageDiv.innerHTML += "<p>description: " + character.data.results[0].description + "</p>"; //prints character description
    }
}
